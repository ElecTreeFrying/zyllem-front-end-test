...
import { ArticleType, ArticleItem, Article } from './common';
...
export class ZyllemApiService {
  ...
  get articles$(): Observable<ArticleItem[]> {
    return of(this._articles).pipe(delay(1000));
  }

  getArticleById$(id: string): Observable<ArticleItem> {
    return this.articles$.pipe(
      map((articles: Article[]) => {
        return articles.find(e => e.id === id);
      })
    );
  }
